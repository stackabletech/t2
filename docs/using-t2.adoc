...
[options="header"]
|=======
|key |description |IONOS Cloud |Amazon EC2
...
|spec.wireguard |(boolean, optional, defaults to `true`) Should a wireguard server be started on the bastion host? Switching wireguard off when you don't need it can speed up the start of the cluster. | | not available
...
|spec.orchestrator.diskType |(optional) type of disk the orchestrator should have | default: `HDD` | default: `gp2` (general purpose SSD), see links below
|spec.orchestrator.diskSizeGb |(optional) size of the disk of the orchestrator in GB | default: `50` | default: `50`
|spec.orchestrator.awsInstanceType |(optional) AWS EC2 instance type | not available | default: `t2.xlarge`, see links below
|spec.nodes |map of node types with their specification 2+|
|spec.nodes.<type>.numberOfNodes |# of nodes of the given type 2+|
|spec.nodes.<type>.numberOfCores |# of cores each node of the given type should have | e.g. `8` | not available
|spec.nodes.<type>.memoryMb |amount of memory each node of the given type should have in MB | e.g. `8192` | not available
|spec.nodes.<type>.diskType | type of disk each node of the given type should have | e.g.: `SSD` | default: `gp2` (general purpose SSD), see links below
|spec.nodes.<type>.diskSizeGb |size of the disk of the given node in GB | e.g. `500` | default: `50`
|spec.nodes.<type>.awsInstanceType |(optional) AWS EC2 instance type | not available | default: `t2.medium`, see links below
