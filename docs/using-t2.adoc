...
|apiVersion |always `t2.stackable.tech/v2` 4+| 
|kind |always `StackableT2Cluster` 4+| 
|metadata.labels |Labels for the cluster. These labels are used for the infrastructure components in the cloud vendors if possible. 4+| 
|spec.template |see section above 4+| 
|spec.domain | (optional) Network domain for cluster-internal use or when accessing through Wireguard VPN (which is currently disabled) 4+| 
|spec.publicKeys | (optional) List of SSH public keys to allow access to cluster nodes. 4+| 
