...
.3+|managed K8s |azure-aks | Microsoft Azure | 
|aws-eks | Amazon AWS | 
...
[options="header"]
|=======
|key |description |Azure AKS|Amazon EKS |IONOS K8S
...
|spec.location |one of the locations that the cloud vendor provides | e.g. `West Europe` 2+| not available 
|spec.region |one of the regions that the cloud vendor provides | not available | e.g. `eu-central-1` | e.g. `de/fra`
|spec.k8sVersion |The K8s version (optional, defaults to whatever the cloud provider regards as the default) 3+| 
|spec.node_count |# of nodes in the cluster 3+|
|spec.vm_size |one of the VM sizes that the cloud vendor provides (optional) | e.g. `Standard_D2_v2` (which is also the default) 2+| not available
|spec.awsInstanceType |(optional) AWS EC2 instance type | not available | default: `t2.xlarge`, see links below | not available
|spec.numberOfCores |(optional) # of cores each node should have 2+| not available | default: `4`
|spec.memoryMb |(optional) amount of memory each node should have in MB 2+| not available | default: `4096`
|spec.diskType |(optional) type of disk each node should have 2+| not available | default: `SSD`
|spec.diskSizeGb |(optional) size of the disk of the given node in GB 2+| not available | default: `250`
|spec.versions |(optional) Map of versions of the Stackable operators to be used in this cluster. See below for a list of Stackable components. 3+|
...
The testdriver operates on a cluster which exists independently from itself. The testdriver neither creates nor terminates any cluster.
