...
echo
sh resources/stackable.sh orchestrator "apt list stackable-* " | grep 'operator-server' | sed 's/stackable-//' | sed 's/-operator-server.*//'
echo
echo
...
echo
sh resources/stackable.sh orchestrator "apt list -a stackable-$operator-operator-server"
echo
